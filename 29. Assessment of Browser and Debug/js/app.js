...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product">
      ...
      <button id="details-btn" class="btn btn-danger" data-toggle="modal" data-target="#myModal" onclick="loadProduct(${product.id})">Details</button></div>
      `;
...
const loadProduct = (id) => {
  const product = loadProducts().find((pd) => pd.id === id);
  ...
  div.innerHTML = `
    <div>
      <img class="product-image img-responsive center-block" src=${product.image}></img>
    </div>
    <h3>${product.title}</h3>
